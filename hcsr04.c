...
#define maxCon 20
#define nch 128 // number of neurons
...
int main(void) {
	...
	for ( ii=0; ii<(nch-10); ii++){
		...
		for (iii=2; iii<maxCon; iii++){
			...
			weights[ii][iii] = random_float((float) -5, (float) 5);
		}
	}
 	...
 	/* Initialize the PRU for LEDS */
  	...
  	for (ii=0; ii<num_pixels; ii++){
  		rgb_spike[ii][0] = (uint8_t) 100+ rand() % 1;
  		rgb_spike[ii][1] = (uint8_t) rand() % 10;
  		rgb_spike[ii][2] = (uint8_t) rand() % 10;

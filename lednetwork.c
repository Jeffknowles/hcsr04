...
const double sense_thresh_i = 180; // threshold where responses turn on
...
#define maxCon 20
#define nch 10050 // number of neurons
...
int main(void) {
	...
	for ( ii=0; ii<(nch-5); ii++){
		...
		for (iii=3; iii<maxCon; iii++){
			...
			weights[ii][iii] = random_float((float) -3, (float) 8);
		}
	...
	while (1) {
		...
		if (time_since_last_ping > currentIPI) {
				...
				printf("%d: Distance = %05.1f cm    loop_spikes = %02d   spike rate = %05.1f Hz \n dt= %f", i, target_distance,loop_spikes, (double) rep_spikes / (double) time_since_last_ping, dt);
			    // target_distance = 90; 

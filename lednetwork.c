...
int main(void) {
	...
	for ( ii=0; ii<(nch-5); ii++){
		...
		for (iii=3; iii<maxCon; iii++){
			...
			weights[ii][iii] = random_float((float) 0, (float) 0);
		}
  ...
  //       printf("\n");
		...
		for (ch = 0; ch < nch; ch++) {
			if (v[ch] >= 0) { // if neuron is in integrate mode
		    	...
		    	ledscape_set_color(frame, 0, ch, (uint8_t) ((double) rgb_spike[ch][0] * v[ch] / (2*thresh)), (uint8_t) ((double) rgb_spike[ch][1] * v[ch] / (2*thresh)), (uint8_t) ((double) rgb_spike[ch][2] * v[ch] / (2*thresh)); 

  ...
  //       printf("\n");
		...
		for (ch = 0; ch < num_sound_inputs; ch++){
			 ...
			 if ((double) ao_values[1] / ao_max >= 0.1 & v[sound_inputs[ch]] >= 0) {
			    v[sound_inputs[ch]] = v[sound_inputs[ch]] + fabs((double) 2*ao_values[0] / ao_max)*(log((double) ao_values[1] / ao_max));
			 }
		}
		...
		for (ch = 0; ch < num_touch_inputs; ch++){
			 ...
			 if ((double) ao_values[1] / ao_max >= 0.1 & v[touch_inputs[ch]] >= 0) {
			    v[touch_inputs[ch]] = v[touch_inputs[ch]] + ((double) 2*ao_values[0] / ao_max)*(10*(double) ao_values[1] / ao_max);
			 }
		}

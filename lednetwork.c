  ...
  //       printf("\n");
		...
		for (ch = 0; ch < nch; ch++) {
			if (v[ch] >= 0) { // if neuron is in integrate mode
		    	...
		    	ledscape_set_color(frame, 0, ch, (uint8_t) ((double) rgb_spike[ch][0] * v[ch] / thresh), (uint8_t) ((double) rgb_spike[ch][1] * v[ch] / thresh), (uint8_t) ((double) rgb_spike[ch][2] * v[ch] / thresh)); 

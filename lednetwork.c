	...
	for (ii=linear_layer_length; ii<(nch); ii++){ // random synapses onto second layer
 	...
 	/* Initialize the PRU for LEDS */
  	...
  	for (ii=0; ii<num_pixels; ii++){
  		rgb_spike[ii][0] = (uint8_t) myrandint( (long int 50));
  		rgb_spike[ii][1] = (uint8_t) myrandint( (long int 50));
  		rgb_spike[ii][2] = (uint8_t) myrandint( (long int 50));
  ...
  //       printf("\n");
		...
		for (ch = 0; ch < nch; ch++) {
			if (v[ch] >= 0) { // if neuron is in integrate mode
		    	...
		    	if (ch < num_pixels){
		    		ledscape_set_color(frame, 0, ch, (uint8_t) 0, (uint8_t) 0,(uint8_t) round(20* v[ch] / (thresh*1.5))); 
			    }
